<script lang="ts">
  ...
  import { getPreviewsByPr, type PrPreviewGroup } from "$lib/api"
  ...
  let liveData = $state<PrPreviewGroup | null>(null)
  // Viewed data - what the user is currently looking at (may be pinned to older SHA)
  let viewedData = $state<PrPreviewGroup | null>(null)
  ...
  // GitHub URL for PR
  const githubUrl = $derived(`https://github.com/${repo}/pull/${prNumber}`)
  ...
  onMount(() => {
    mounted = true
    if (browser && org && repo && prNumber > 0) {
      loadData()
      connectStream()
    }
  ...
  async function loadData() {
    ...
    error = ""
    try {
      const res = await getPreviewsByPr(org, repo, prNumber)
      liveData = res.data
  ...
  function connectStream() {
    ...
    const url = `/api/orgs/${encodeURIComponent(org)}/repos/${encodeURIComponent(repo)}/pr/${prNumber}/stream${params}`
    ...
    es.addEventListener("update", (event) => {
      try {
        const payload = JSON.parse((event as MessageEvent).data) as { data: PrPreviewGroup | null }
        if (payload.data) {
...
{:else if error}
  <div class="flex items-center justify-center h-full">
    <div class="bg-red-950/50 border border-red-800 rounded px-6 py-4 text-sm text-red-300 max-w-md">
      <div class="font-medium mb-1">Error loading PR</div>
