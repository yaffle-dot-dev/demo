...
import CopyButton from "$lib/components/CopyButton.svelte"
  import { getOrgConnection, listEnvironments } from "$lib/api"
  import type { EnvironmentGroup, OrgConnection } from "$lib/api"

  type SettingsSection = "members" | "connections" | "support"
  ...
  let activeSection: SettingsSection = $state("connections")
...
<div class="grid gap-10 lg:grid-cols-[180px_minmax(0,1fr)]">
  <aside class="space-y-6">
    <nav class="space-y-1">
      {#each navItems as item}
        <button
          class={`block w-full rounded-lg px-3 py-2 text-left text-sm transition ${activeSection === item.id
  ...
  <div class="min-w-0 space-y-8">
    {#if activeSection === "connections"}
      <section class="space-y-2 border-b border-border pb-6">
        <div class="flex items-end justify-between gap-4">
          ...
          <button
            class="rounded-lg border border-border px-3 py-2 text-sm text-text transition hover:bg-surface-raised"
    ...
    {:else if activeSection === "members"}
      <section class="space-y-2 border-b border-border pb-6">
        <h2 class="text-2xl font-semibold text-text">Members</h2>
        <p class="text-sm text-text-muted">
          Roles, ownership, and audit visibility will live here.
        </p>
      </section>

      <div class="border border-dashed border-border rounded-lg p-6 text-sm text-text-dim">
        Member management prototype coming next.
      </div>
    {:else}
      <section class="space-y-2 border-b border-border pb-6">
        <h2 class="text-2xl font-semibold text-text">Support</h2>
        <p class="text-sm text-text-muted">
          Operational help, incidents, and onboarding guidance will live here.
        </p>
      </section>

      <div class="grid gap-4 md:grid-cols-2">
        <div class="border border-border rounded-lg p-4">
          <div class="text-sm font-medium text-text">Connection setup help</div>
          <div class="mt-1 text-sm text-text-muted">Guided help for common provider setup flows.</div>
        </div>

        <div class="border border-border rounded-lg p-4">
          <div class="text-sm font-medium text-text">Escalation</div>
          <div class="mt-1 text-sm text-text-muted">Platform support and troubleshooting workflow.</div>
        </div>
      </div>
    {/if}
  </div>
