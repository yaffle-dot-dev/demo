...
function mockConnection(
  ...
  overrides: Partial<OrgConnection> = {},
): OrgConnection {
  return {
    ...
    providerType:
      ...
      (typeof config.providerType === "string" ? config.providerType : null),
    credentialProviderType:
      overrides.credentialProviderType ??
...
function baseInput(
  overrides: Partial<ConnectionScopeValidationInput> = {},
): ConnectionScopeValidationInput {
  return {
    connections: [],
    providerSignatures,
    missingRequirements: [],
...
describe("connection-scope-validation", () => {
  test("shows known scopes from existing connections and current selections", () => {
    const result = buildConnectionScopeValidation(
      baseInput({
        connections: [
          mockConnection({
            providerType: "aws",
  ...
  test("allows selecting scopes before credential fields are known", () => {
    ...
    expect(result.conflictMessages).toEqual([])
    expect(result.environmentOptions.find((option) => option.value === "main")?.conflicting).toBe(
      false,
    )
    expect(
      result.workspaceOptions.find((option) => option.value === "apps/infra")?.conflicting,
    ).toBe(false)
  })
