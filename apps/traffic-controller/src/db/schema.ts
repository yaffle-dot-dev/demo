...
export const routeableDeploymentReceiverKindEnum = trafficControl.enum(
  "routeable_deployment_receiver_kind",
  ["github_webhook"],
)
...
export const routeableDeployments = trafficControl.table(
  "routeable_deployments",
  {
    id: uuid("id")
      .primaryKey()
      .$defaultFn(() => uuidv7()),
    ...
    receiverKind: routeableDeploymentReceiverKindEnum("receiver_kind")
...
export const liveWebhookLeases = trafficControl.table(
  "live_webhook_leases",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (t) => [
    ...
    uniqueIndex("tc_live_webhook_leases_active_exact_scope_idx")
      .on(t.event, t.installationId, t.repositoryId, t.action, t.pullRequestNumber, t.ref)
      .where(sql`${t.status} = 'active'`),
  ],
)

export const trafficControlOperations = trafficControl.table(
  "operations",
  {
    id: uuid("id")
      .primaryKey()
    ...
    routeableDeploymentId: uuid("routeable_deployment_id").references(
      () => routeableDeployments.id,
      { onDelete: "cascade" },
    ),
    liveWebhookLeaseId: uuid("live_webhook_lease_id").references(() => liveWebhookLeases.id, {
      onDelete: "cascade",
    }),
...
export const trafficControlAuditEvents = trafficControl.table(
  "audit_events",
  {
    id: uuid("id")
      .primaryKey()
      .$defaultFn(() => uuidv7()),
    operationId: uuid("operation_id").references(() => trafficControlOperations.id, {
      onDelete: "set null",
    }),
    routeableDeploymentId: uuid("routeable_deployment_id").references(
      () => routeableDeployments.id,
