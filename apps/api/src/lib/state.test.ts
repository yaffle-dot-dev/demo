...
describe("stateExists", () => {
  test("returns false when no state file exists", () => {
    expect(stateExists(TEST_OWNER, TEST_REPO, "previews/pr-0/terraform.tfstate")).toBe(false)
  })
  ...
  test("returns true when state file exists", async () => {
    ...
    expect(stateExists(TEST_OWNER, TEST_REPO, stateKey)).toBe(true)
  })
})
...
describe("removeState", () => {
  test("removes the state directory", async () => {
    ...
    await removeState(TEST_OWNER, TEST_REPO, stateKey)
  ...
  test("does not throw when directory does not exist", async () => {
    // Should not throw
    await removeState(TEST_OWNER, TEST_REPO, "previews/pr-0/terraform.tfstate")
  })
})
