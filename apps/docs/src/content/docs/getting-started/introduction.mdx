...
<Aside type="note" title="OpenTofu, not Terraform">
  Yaffle runs [OpenTofu](https://opentofu.org), the open-source fork of Terraform under MPL 2.0.
  OpenTofu is fully compatible with existing Terraform configurations and uses the same provider
  ecosystem. We chose OpenTofu to give you infrastructure automation without vendor lock-in.
</Aside>
