...
export const iacJobs = pgTable(
  ...
  (table) => [
    index("iac_jobs_queue_priority_idx").using(
      "btree",
      table.status.asc().nullsLast().op("text_ops"),
      table.jobType.asc().nullsLast().op("text_ops"),
      table.queuedAt.asc().nullsLast().op("text_ops"),
    ),
    foreignKey({
      columns: [table.previewId],
      foreignColumns: [workspaceDeployments.id],
...
export const workspaceDeployments = pgTable(
  ...
  (table) => [
    index("workspace_deployments_upstream_ids_gin").using(
      "gin",
      table.upstreamIds.asc().nullsLast().op("array_ops"),
    ),
    foreignKey({
      columns: [table.orgId],
      foreignColumns: [organizations.id],
