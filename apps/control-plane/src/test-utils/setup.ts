...
if (process.env.YAFFLE_SKIP_TEST_DB_SETUP === "true") {
  ...
  if (!process.env.TRUSTED_ORIGINS) {
    process.env.TRUSTED_ORIGINS =
      "https://yaffle.local:6969,http://yaffle.local:5173,http://yaffle.local:3000"
  }
  ...
  if (
    currentUrl.includes(DEV_DB_NAME) ||
    (!currentUrl.includes(TEST_DB_NAME) && !currentUrl.includes("_test"))
  ) {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[test-setup] Migration state was inconsistent; resetting test schemas and retrying",
      )
    ...
    const { ensureDefaultProviderCredentialSignatures } =
      await import("../db/queries/provider-credential-signatures.ts")
